...
var usernameTimer, passwordTimer, emailTimer, captchaTimer, validStr = String.fromCharCode(10003);
...
function validateCaptcha() {
	if (captchaField.value.match(/^[a-z0-9 ]{3,24}$/i)) {
		captchaMsg.nodeValue = validStr;
		captchaMsg.parentNode.style.color = '#00DD00';
	} else {
		captchaMsg.nodeValue = 'That\'s not a valid answer.';
		captchaMsg.parentNode.style.color = '#FF0000';
	}
}
...
function validate(input) {
	...
	else if (input == captchaField) {
		clearTimeout(captchaTimer);
		if (shouldValidate(input, input.nextSibling.nextSibling)) captchaTimer = setTimeout(validateCaptcha, 1000);
	}
}

function allValidated() {
	if (usernameMsg.nodeValue !== validStr) validateUsername();
	if (passwordMsg.nodeValue !== validStr || password2Msg.nodeValue !== validStr) validatePassword();
	if (emailMsg.nodeValue !== validStr) validateEmail();
	if (captchaMsg.nodeValue !== validStr) validateCaptcha();
	return (usernameMsg.nodeValue == validStr && passwordMsg.nodeValue == validStr && password2Msg.nodeValue == validStr && emailMsg.nodeValue == validStr && captchaMsg.nodeValue == validStr)
}
...
function initRegister() {
	...
	captchaField = document.getElementsByName('recaptcha_response')[0];
	captchaMsg = document.createElement('span').appendChild(document.createTextNode(''))
	captchaField.parentNode.insertBefore(captchaMsg.parentNode, captchaField.nextSibling.nextSibling);
